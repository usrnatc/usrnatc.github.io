// pips_wasm.cpp
// Compile: emcc pips_wasm.cpp -O2 -o pips.js \
//   -s EXPORTED_FUNCTIONS="['_SolvePuzzleStr','_malloc','_free']" \
//   -s EXPORTED_RUNTIME_METHODS="['ccall','cwasm','UTF8ToString','stringToUTF8','lengthBytesUTF8']" \
//   -s MODULARIZE=1 -s EXPORT_NAME="PipsModule" \
//   -s ALLOW_MEMORY_GROWTH=1 -s TOTAL_STACK=1048576
...
enum RuleType {
    RULE_NONE = 0, RULE_EQ, RULE_NEQ, RULE_GT, RULE_LT, RULE_SUM
};
...
struct Node {
    Node* Left; Node* Right; Node* Up; Node* Down; Node* Column;
    u32 Count; i16 RowID; i16 ColID;
};
...
AddRow(DLX* Dlx, u16 RowIdx, u16 ColIdx[3]) {
    if (Dlx->NodesUsedCount + 3 > MAX_DLX_NODES) return;
...
static void Cover(Node* ColHeader) {
...
CheckAndApplyCell(Region* R, u8 Val) {
    if (R->Type == RULE_EQ && R->FilledCells > 0 && Val != R->FirstVal) return FALSE;
    if (R->Type == RULE_NEQ && (R->Mask & (1 << Val))) return FALSE;
    if (R->Type == RULE_GT) {
        if (R->FilledCells + 1 == R->TotalCells && R->Sum + Val <= R->Target) return FALSE;
    }
    if (R->Type == RULE_LT) {
        if (R->Sum + Val >= R->Target) return FALSE;
    }
    if (R->Type == RULE_SUM) {
        if (R->Sum + Val > R->Target) return FALSE;
        if (R->FilledCells + 1 == R->TotalCells && R->Sum + Val != R->Target) return FALSE;
    }
    if (!R->FilledCells) R->FirstVal = Val;
    R->Mask |= (1 << Val);
    R->Sum += Val;
    ++R->FilledCells;
    return TRUE;
...
static inline void UndoCell(Region* R, u8 Val) {
    --R->FilledCells;
    R->Sum -= Val;
    R->Mask &= ~(1 << Val);
}

static inline b8 ApplyDomino(WorkOrder* Order, u16 RowID) {
    ...
    if (!CheckAndApplyCell(Reg1, M->Val1)) return FALSE;
    if (!CheckAndApplyCell(Reg2, M->Val2)) { UndoCell(Reg1, M->Val1); return FALSE; }
    return TRUE;
}

static inline void RemoveDomino(WorkOrder* Order, u16 RowID) {
    RowMeta* M = &Order->MetaData[RowID];
    Region* Reg2 = &Order->Regions[Order->CellToRegion[M->R2 * Order->GridW + M->C2]];
    Region* Reg1 = &Order->Regions[Order->CellToRegion[M->R1 * Order->GridW + M->C1]];
...
static u8 Solve(DLX* Dlx, WorkOrder* Order) {
    if (Dlx->Head.Right == &Dlx->Head) return TRUE;
    ...
    if (ColToCover->Count > 1) {
        for (Node* T = ColToCover->Right; T != &Dlx->Head; T = T->Right) {
            if (T->Count < ColToCover->Count) {
                ColToCover = T;
                if (ColToCover->Count <= 1) break;
            }
        }
    }
    if (!ColToCover->Count) return FALSE;
    Cover(ColToCover);
    for (Node* RN = ColToCover->Down; RN != ColToCover; RN = RN->Down) {
        if (ApplyDomino(Order, RN->RowID)) {
            Dlx->Solution[Dlx->SolutionSize++] = RN->RowID;
            for (Node* HN = RN->Right; HN != RN; HN = HN->Right) Cover(HN->Column);
            if (Solve(Dlx, Order)) return TRUE;
            --Dlx->SolutionSize;
            for (Node* HN = RN->Left; HN != RN; HN = HN->Left) Uncover(HN->Column);
...
// --- Parse + Solve (adapted from SolvePuzzle, single-puzzle, no threading) ---
...
ParseAndSolve(const char* Line) {
    ...
    while (Line[Cursor] != '\n' && Line[Cursor] != '\r' && Line[Cursor] != '\0') {
        ...
        if (Line[Cursor] == '\0' || Line[Cursor] == '\n' || Line[Cursor] == '\r') {
            Order->Regions[RegID].Type = RULE_NONE; break;
        } else if (Line[Cursor] == '=') {
            Order->Regions[RegID].Type = RULE_EQ; Cursor++;
        } else if (Line[Cursor] == '!' && Line[Cursor+1] == '=') {
            Order->Regions[RegID].Type = RULE_NEQ; Cursor += 2;
        } else if (Line[Cursor] == '>') {
            Order->Regions[RegID].Type = RULE_GT; ++Cursor;
            Order->Regions[RegID].Target = Line[Cursor++] - '0';
        } else if (Line[Cursor] == '<') {
            Order->Regions[RegID].Type = RULE_LT; ++Cursor;
            Order->Regions[RegID].Target = Line[Cursor++] - '0';
        } else if (Line[Cursor] == ' ') {
            Order->Regions[RegID].Type = RULE_NONE; ++Cursor;
        } else {
            Order->Regions[RegID].Type = RULE_SUM;
            u8 Sum = 0;
            while (Line[Cursor] >= '0' && Line[Cursor] <= '9') {
                Sum = Sum * 10 + (Line[Cursor] - '0'); ++Cursor;
            }
    ...
    for (u8 D = 0; D < Order->NumDominoes; ++D) {
        ...
        for (u8 R = 0; R < Order->GridH; ++R) {
            for (u8 C = 0; C < Order->GridW; ++C) {
                if (Order->CellToRegion[R * Order->GridW + C] == 255) continue;
                // Horizontal
                if (C + 1 < Order->GridW && Order->CellToRegion[R * Order->GridW + C + 1] != 255) {
                    Order->MetaData[CurrentRowID] = {R, C, PipA, R, (u8)(C+1), PipB, D};
                    u16 Cols[3] = { (u16)(R * Order->GridW + C), (u16)(R * Order->GridW + C + 1), DominoColIdx };
                    AddRow(Dlx, CurrentRowID++, Cols);
                    if (PipA != PipB) {
                        Order->MetaData[CurrentRowID] = {R, C, PipB, R, (u8)(C+1), PipA, D};
                        AddRow(Dlx, CurrentRowID++, Cols);
                    }
                }
                // Vertical
                if (R + 1 < Order->GridH && Order->CellToRegion[(R+1) * Order->GridW + C] != 255) {
                    Order->MetaData[CurrentRowID] = {R, C, PipA, (u8)(R+1), C, PipB, D};
                    u16 Cols[3] = { (u16)(R * Order->GridW + C), (u16)((R+1) * Order->GridW + C), DominoColIdx };
                    AddRow(Dlx, CurrentRowID++, Cols);
                    if (PipA != PipB) {
                        Order->MetaData[CurrentRowID] = {R, C, PipB, (u8)(R+1), C, PipA, D};
                        AddRow(Dlx, CurrentRowID++, Cols);
                    }
                }
            }
    ...
    if (Solve(Dlx, Order)) {
        for (u16 I = 0; I < Order->NumCells; ++I) {
            if (Order->CellToRegion[I] == 255) Order->Board[I] = '0';
        }
...
const char* SolvePuzzleStr(const char* PuzzleLine) {
    if (!PuzzleLine || !PuzzleLine[0]) return 0;
